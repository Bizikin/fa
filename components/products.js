...
export default function Products() {
  ...
  function chooseProduct(categoryId) {
    axios(
      `${process.env.NEXT_PUBLIC_API_URL}api/product?restaurantId=${
        state.id ? state.id : 1
  ...
  function showIncDecCart(productId) {
    axios
      .put(
        `${process.env.NEXT_PUBLIC_API_URL}api/basket/${productId}`,
        {},
  ...
  function incCart(productId) {
    axios
      .put(
        `${process.env.NEXT_PUBLIC_API_URL}api/basket/${productId}/inc`,
        {},
        { withCredentials: true }
      )
      .then((res) => {
  ...
  function decCart(productId) {
    axios
      .put(
        `${process.env.NEXT_PUBLIC_API_URL}api/basket/${productId}/dec`,
        {},
        { withCredentials: true }
      )
      .then((res) => {
      ...
      <div id="category" className="category">
        {state.categories &&
          state.categories.map((category) => (
            <div
              key={category.id}
              onClick={() => chooseProduct(category.id)}
            ...
            <div className="product" key={product.id}>
              ...
              {!(
                ...
                <div className="product__box">
                  ...
                  <div
                    className="basket"
                    onClick={() => showIncDecCart(product.id)}
                  >
